  ...
  <xsl:template match="r:DebitNote">
    <html>
      ...
      <body>
        ...
        <!-- Inicio Variables Documento Nota de Débito -->
        ...
        <xsl:variable name="Campo42_OtrosCargos">
          <xsl:value-of select="/*/cac:RequestedMonetaryTotal/cbc:ChargeTotalAmount"/>
        </xsl:variable>
        ...
        <xsl:variable name="Campo46_ImporteTotal">
          <xsl:variable name="Temp">
            <xsl:value-of select="format-number(//cac:RequestedMonetaryTotal/cbc:PayableAmount,'#,##0.00','euro')"/>
          </xsl:variable>
          <xsl:choose>
            <xsl:when test="$Temp!='' and $Temp!='NaN'">
              <xsl:value-of select="$Temp"/>
        ...
        <!-- Fin Variables Documento Nota de Débito -->
